...
def test_total_edge_negative_logs(monkeypatch) -> None:
    from bot.strategy import mm_funding as module

    monkeypatch.setattr(module.book_md, "snapshot_from_store", _snapshot_from_store)
    monkeypatch.setattr(module.book_md, "bbo_from_snapshot", lambda snapshot: SimpleNamespace(bid=100.0, ask=100.2, bid_size=1.0, ask_size=1.0, ts=snapshot.ts))
    ...
    funding_cache = SimpleNamespace(last=FundingInfo(funding_rate=0.000051, next_update_time=None, interval_sec=None, ts=time.time()))
    oms = DummyOMS()
    logger = DummyLogger()
    ...
    assert "edge_negative_total" in oms.cancel_reasons
    risk_logs = [record for record in logger.records if record.get("reason") == "edge_negative_total"]
    assert risk_logs
    risk_log = risk_logs[0]
