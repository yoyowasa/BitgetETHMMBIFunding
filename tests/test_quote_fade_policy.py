...
class DummyOMS:
    def __init__(self, mid_move_bps: float) -> None:
        prev_mid = MID / (1.0 + mid_move_bps / 10000.0)
        self.gateway = SimpleNamespace(
            ...
            mid_100ms_ago=lambda now=None: prev_mid,
        )
...
def _config(policy: str | None = None) -> AppConfig:
    ...
    if policy is not None:
        kwargs["quote_fade_policy"] = policy
    return AppConfig(
...
def _run(monkeypatch, *, policy: str, mid_move_bps: float) -> tuple[DummyOMS, DummyLogger]:
    ...
    oms = DummyOMS(mid_move_bps=mid_move_bps)
    logger = DummyLogger()
...
def _logs(logger: DummyLogger, reason: str) -> list[dict]:
    return [record for record in logger.records if record.get("reason") == reason]


def test_quote_fade_policy_default_is_current() -> None:
    assert StrategyConfig(
        ...
        base_half_spread_bps=18.0,
        quote_refresh_ms=250,
    ).quote_fade_policy == "current"


def test_quote_fade_policy_loads_all_values(tmp_path) -> None:
    for policy in ("disabled", "threshold_5bps", "threshold_8bps", "threshold_10bps"):
...
""",
        ...
        assert load_config(str(path)).strategy.quote_fade_policy == policy


def test_current_keeps_existing_quote_fade(monkeypatch) -> None:
    oms, logger = _run(monkeypatch, policy="current", mid_move_bps=4.0)

    assert oms.cancel_reasons == ["quote_fade"]
    assert oms.last_update_quotes is None
    assert _logs(logger, "quote_fade")
    assert not _logs(logger, "quote_fade_suppressed")


def test_disabled_suppresses_quote_fade(monkeypatch) -> None:
    oms, logger = _run(monkeypatch, policy="disabled", mid_move_bps=12.0)

    assert oms.cancel_reasons == []
    assert oms.last_update_quotes is not None
    suppressed = _logs(logger, "quote_fade_suppressed")
    assert suppressed
    assert suppressed[0]["quote_fade_policy"] == "disabled"


def test_threshold_5bps_suppresses_below_and_passes_at_threshold(monkeypatch) -> None:
    oms_low, logger_low = _run(monkeypatch, policy="threshold_5bps", mid_move_bps=4.9)
    assert oms_low.cancel_reasons == []
    assert _logs(logger_low, "quote_fade_suppressed")

    oms_high, logger_high = _run(monkeypatch, policy="threshold_5bps", mid_move_bps=5.0)
    assert oms_high.cancel_reasons == ["quote_fade"]
    assert _logs(logger_high, "quote_fade")


def test_threshold_8bps_suppresses_below_and_passes_at_threshold(monkeypatch) -> None:
    oms_low, logger_low = _run(monkeypatch, policy="threshold_8bps", mid_move_bps=7.9)
    assert oms_low.cancel_reasons == []
    assert _logs(logger_low, "quote_fade_suppressed")

    oms_high, logger_high = _run(monkeypatch, policy="threshold_8bps", mid_move_bps=8.01)
    assert oms_high.cancel_reasons == ["quote_fade"]
    assert _logs(logger_high, "quote_fade")


def test_threshold_10bps_suppresses_below_and_passes_at_threshold(monkeypatch) -> None:
    oms_low, logger_low = _run(monkeypatch, policy="threshold_10bps", mid_move_bps=9.9)
    assert oms_low.cancel_reasons == []
    assert _logs(logger_low, "quote_fade_suppressed")

    oms_high, logger_high = _run(monkeypatch, policy="threshold_10bps", mid_move_bps=10.01)
    assert oms_high.cancel_reasons == ["quote_fade"]
    assert _logs(logger_high, "quote_fade")
