...
def test_min_half_spread_guard(monkeypatch) -> None:
    from bot.strategy import mm_funding as module

    monkeypatch.setattr(module.book_md, "snapshot_from_store", _snapshot_from_store)
    monkeypatch.setattr(module.book_md, "bbo_from_snapshot", lambda snapshot: SimpleNamespace(bid=100.0, ask=100.2, bid_size=1.0, ask_size=1.0, ts=1.0))
    ...
    bid_px = oms.last_update_quotes["bid_px"]
    ask_px = oms.last_update_quotes["ask_px"]
    spread_bps = (ask_px - bid_px) / ((ask_px + bid_px) / 2.0) * 10000.0
    assert spread_bps >= 15.99
    assert any(record.get("reason") == "spread_below_min" for record in logger.records)
