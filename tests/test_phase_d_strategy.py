...
def test_carry_exit_holds_fee_only_loss_inside_funding_window(monkeypatch) -> None:
    ...
    config.strategy.carry_exit_max_loss_bps = 5.0
    config.strategy.carry_exit_min_hold_sec = 0.0
    oms = DummyOMS(spot_available=1634.36)
    oms.positions.spot_pos = 1634.364
    oms.positions.perp_pos = -1636.0
    oms.carry_snapshot = SimpleNamespace(
    ...
    logger = DummyLogger()
    strategy = MMFundingStrategy(config, funding_cache, oms, DummyRisk(), logger)
    monkeypatch.setattr(strategy, "_in_funding_window", lambda now: True)
    ...
    asyncio.run(
        strategy._maybe_exit_carry_position(
            now=time.time(),
            spot_bbo=SimpleNamespace(bid=0.03655, ask=0.03656),
            perp_bbo=SimpleNamespace(bid=0.03666, ask=0.03667),
            funding_rate=0.000578,
            basis=0.00011,
            obi_spot=0.0,
            obi_perp=0.0,
